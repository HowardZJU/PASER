...
class Trainer:
    ...
    def train(self):
        early_stopping = EarlyStopping(
            self.path + "/model.pt", patience=self.args.patience
        )
        ...
        for epoch in range(self.args.epochs):
            ...
            if full_sort:
                metric_dict = {
                    ...
                    "val/HIT@30": scores[6],
                    "val/NDCG@30": scores[7],
                    "val/HIT@50": scores[8],
                    "val/NDCG@50": scores[9]
                ...
                metric_dict = {
                    ...
                    "val/NDCG@5": scores[3],
                    "val/HIT@10": scores[4],
                    "val/NDCG@10": scores[5],
                    "val/MRR": scores[6],
                }
            [
                self.writer.add_scalar(key, metric_dict[key], global_step=epoch)
        ...
        if full_sort:
            metric_dict = {
                ...
                "test/HIT@10": scores[2],
                "test/NDCG@10": scores[3],
                "test/HIT@20": scores[4],
                "test/NDCG@20": scores[5],
                "test/HIT@30": scores[6],
                "test/NDCG@30": scores[7],
                "test/HIT@50": scores[8],
                "test/NDCG@50": scores[9]
            ...
            metric_dict = {
                "test/HIT@1": scores[0],
                "test/NDCG@1": scores[1],
                "test/HIT@5": scores[2],
                "test/NDCG@5": scores[3],
                "test/HIT@10": scores[4],
                "test/NDCG@10": scores[5],
                "test/MRR": scores[6],
            }
    ...
    def get_full_sort_score(self, epoch, answers, pred_list):
        ...
        result_info = (
            ...
            + "HIT@20:{:.4f} ".format(recall[2])
            + "HIT@30:{:.4f} ".format(recall[3])
            + "HIT@50:{:.4f} ".format(recall[4])
            + "NDCG@5:{:.4f} ".format(ndcg[0])
            + "NDCG@10:{:.4f} ".format(ndcg[1])
            + "NDCG@20:{:.4f} ".format(ndcg[2])
            + "NDCG@30:{:.4f} ".format(ndcg[3])
    ...
    def cross_entropy(self, seq_out, pos_ids, neg_ids):
        ...
        #     pos_emb = pos_emb.div(torch.norm(pos_emb, p=2, dim=-1, keepdim=True) + 1e-8)
        #     neg_emb = neg_emb.div(torch.norm(neg_emb, p=2, dim=-1, keepdim=True) + 1e-8)
...
class PASTrainer(Trainer):
    def __init__(self, model, train_dataloader, eval_dataloader, test_dataloader, args):
        super(PASTrainer, self).__init__(
    ...
    def visualize(self, dataloader):
        for i, batch in enumerate(dataloader):
            ...
            self.writer.add_graph(self.model, (input_ids,))
            break
