...
sequential_data_list = ['Beauty','Sports_and_Outdoors','Toys_and_Games','Yelp', 'Bus', 'Taobao']
...
class EarlyStopping:
    ...
    def __init__(self, checkpoint_path, patience=10, delta=0):
        """
        Args:
...
def get_dataloder(args,seq_dic):
    ...
    if args.data_name in sequential_data_list:
        ...
        train_dataloader = DataLoader(train_dataset, sampler=train_sampler, batch_size=args.batch_size, num_workers=num_workers)

        eval_dataset = FMLPRecDataset(args, seq_dic['user_seq'], test_neg_items=seq_dic['sample_seq'], data_type='valid')
        eval_sampler = SequentialSampler(eval_dataset)
        eval_dataloader = DataLoader(eval_dataset, sampler=eval_sampler, batch_size=args.batch_size, num_workers=num_workers)

        test_dataset = FMLPRecDataset(args, seq_dic['user_seq'], test_neg_items=seq_dic['sample_seq'], data_type='test')
        test_sampler = SequentialSampler(test_dataset)
        test_dataloader = DataLoader(test_dataset, sampler=test_sampler, batch_size=args.batch_size, num_workers=num_workers)
